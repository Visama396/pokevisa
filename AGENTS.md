...
- Scoring: larger matched patterns swallow smaller contained ones; JACKPOT is
  exempt both ways. Payout = Σ symbolValue×cells×patternMult × (1+global)
...
- Sprites are self-hosted copies of PokeAPI assets in `public/slots/`
  ...
  symbols and Shiny charms; `items/<name>.png` = charm icons), so a run
...
- Economy rework (Aug 2026): quotas now follow a fixed exponential table
  `QUOTA_TABLE = [75,200,666,2222,12500,33333,66666,200000,1000000,6000000,144e6,13.8e9,2e13,2e17,2e22,...]` (×10/quota beyond) and round entry fees are fixed per quota (`ROUND_COST_TABLE=[7,14,28,42,56,140,168,196,224,756,1260,1386,1500]`; 3 pulls ≈35% of the 7-pull fee; extrapolates +10%/quota). 70 starting coins unchanged. Reroll prices double forever (`rerollCost`, never resets; `rerollsThisCycle` tracks per-quota rerolls for Pink Scarf).
- Wave 1 (+14 charms, 9-lang): Focus Sash (deadline +2 rounds, then discard), Lagging Tail (doubles every `trait:"random"` charm — Amulet Coin 10%→20%, Odd Keystone 35%→70%, Parcel 25%→50% via `randomTriggerChance`), Arceus Statue (next pull guaranteed JACKPOT then self-discards, sprite `pokemon/493.png`), Black Sludge (every discarded charm — sold or consumed — feeds `permLevels` +base value to all symbols via `discardCharm`), Metronome (emptyStreak≥2 → pending Luck +5 +2/consecutive fire), Heal Powder (lone pattern → transform its symbols to `symbolValue`-max), Zoom Lens (`patternMult` +1/15 tickets), Guidebook (on quota clear `upgrades.global += roundsLeft+1`), Star Piece (prior pull empty → center-row HOR-XL forced), Odd Keystone (`trait:"random"` payout×2), Bright Powder (5 `itemTriggers`/round → pending Luck+7 once/round), Deep Sea Tooth (3 pulls without a 5+-cell pattern → ABOVE/BELOW forced), Point Card (`upgrades.global += rerolls` retroactive +1/reroll), Parcel (`trait:"random"`). Trait shown colored ⚡ in tray + shop tooltips.
- Wave 2 (+24 charms; Choice Specs reworked: now cost 2, on `emptyStreak===3` permanently raises every pattern by its own base mult via `permPatternLevels` — old DIAG/ZIG/ZAG+2 removed; Choice Scarf (cost 2, same streak raises all symbols via `permLevels`); 4 Gen-5 drives keyed off every 7th pull (`DRIVE_EVERY_PULLS=7`: Douse=payout again, Shock=Luck+7, Burn=forced ZIG_ZAG, Chill=+`CHILL_DRIVE_EXTRA_PULLS`=2 pulls/round, dynamic `BASE_MAX_PULL_SLOTS`); 4 mints (emoji-only, no PokeAPI sprites): Modest (+1 ticket per `cycle` tickets at quota clear), Bold (+1 Symbols Mult level per 5 tickets via `symbolsMultLevel`), Adamant (per pull past 2nd pattern `adamantStacks` +base values, round-scoped), Sassy (≥5 patterns → `sassyDoubles` doubles symbols×2/round-scoped); 4 terrain seeds: Electric (×4+ pattern → `electricBoost` +base mults/round), Psychic (×5+ → `psychicDoubles` ×2/round), Misty (EYE alone → `mistyDoubles` ×2 permanent), Grassy (alternating +1 Symbols/Patterns levels `grassyLevels`/`grassyTurns` per cleared quota); Leftovers (lemon/cherry patterns pay twice, self-discards after 10 completed rounds via `tickLeftovers` at the 4 `totalRounds` sites), Poison Barb (≥7 Seviper cells → all become Shiny `goldenCells` union), Twisted Spoon (lone diamond/treasure → +3×payout, total 4×), 5 contest scarves `trait:"random"` (`scarf:{luck,chance,maxFires?,source?,step?}`; Red 20% +5 Luck/12 fires → discard, Blue 15% +7/9 fires, Pink 10% Luck=2×`rerollsThisCycle` until quota end, Green 10% Luck=`greenScarfBonus` which grows +3 per ×3+ pattern when it fires, Yellow 10% Luck=`yellowScarfBonus`=4×roundsSkippedPrevQuota until quota end — all doubled by Lagging Tail; maxFires handled via `scarfFires` map, bonus via `greenScarfBonus`/`yellowScarfBonus`/`rerollsThisCycle`/`scarfFired` temp object).
- Round-scoped resets in `chooseRoundMode` (itemTriggers, brightPowderFired, adamantStacks, sassyDoubles, electricBoost, psychicDoubles); quota-scoped resets at cycle advance (rerollsThisCycle, greenScarfBonus) plus Yellow setup; Mid-cycle `tickLeftovers` at `base.roundsLeft>0` branch.
- Board footers: Symbols Multiplier `×symbolsMult(state)` (`1+global+Bold+GrassyGlobal`) and Patterns Multiplier `×patternsMult(state)` (`1+grassyPattern+ZoomLens`) shown under the boards; translation keys `Symbols Multiplier` (exists) + new `Patterns Multiplier` (9 langs).
- 60 charms total; shop filters `poffin-case` once via `poffinCaseBought`; tray capacity `charmSlots` counts poffin without occupying; generated via `generateShopOffers`; win still endless debt ×2.
