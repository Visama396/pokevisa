...
- Board footers: Symbols Multiplier `×symbolsMult(state)` (`1.5^global+Bold+GrassyGlobal`) and Patterns Multiplier `×patternsMult(state)` (`1+grassyPattern+ZoomLens`) shown under the boards; translation keys `Symbols Multiplier` (exists) + new `Patterns Multiplier` (9 langs).
